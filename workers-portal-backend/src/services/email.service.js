...
async function _sendOtpSms(phone, otp) {
  const message = `Your Smart Workers OTP is ${otp}. Valid for 10 minutes. Do not share with anyone.`;
...
async function sendOtpEmail(email, otp) {
  return _sendEmail({
    to: email,
    subject: 'Smart Workers — Your OTP',
...
async function sendBookingConfirmation(customerEmail, customerPhone, booking, workerName) {
  ...
  if (customerEmail) {
    _sendEmail({
      ...
      html: `<div style="font-family:sans-serif;max-width:500px;margin:0 auto;padding:32px">
        <h2 style="color:#4338ca;margin:0 0 4px">Smart Workers</h2>
        <h3 style="color:#065f46;margin:0 0 20px">✅ Booking Confirmed!</h3>
        ${_table(rows)}
        <p style="color:#9ca3af;font-size:13px;margin-top:20px">Thank you for choosing Smart Workers!</p>
      </div>`,
    }).catch(() => {});
  }
  ...
  if (customerPhone) {
    _sendSms(customerPhone,
      `Smart Workers: Booking confirmed! ${workerName} (${booking.job_type}) on ${dateStr}. Total: Rs.${booking.total_price}. Address: ${booking.address || 'provided'}`
    ).catch(() => {});
  }
}
...
async function sendWorkerBookingAlert(workerEmail, workerPhone, workerName, booking, customerName) {
  ...
  if (workerPhone) {
    _sendSms(workerPhone,
      `Smart Workers: New booking! ${customerName} booked you for ${booking.job_type} on ${dateStr}. Address: ${booking.address || 'to be shared'}. Amount: Rs.${booking.total_price}`
    ).catch(() => {});
  }
}
